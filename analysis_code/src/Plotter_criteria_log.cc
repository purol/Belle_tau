...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    if (argc == (7 + 2 * variable_num)) {
        Module::Module* temp_module = new Module::ConditionalPairDrawStack(variable_pair, condition_order, (";" + std::string(argv[6 + 2 * variable_num]) + ";arbitrary unit").c_str(), (argv[5 + 2 * variable_num] + std::string("/") + argv[6 + 2 * variable_num] + ".png").c_str(), true, true, *loader.SignalLabel_address(), *loader.BackgroundLabel_address(), *loader.DataLabel_address(), *loader.MCLabel_address(), loader.Getvariable_names_address(), loader.VariableTypes_address());
        loader.InsertCustomizedModule(temp_module);
    }
    else if (argc == (9 + 2 * variable_num)) {
        Module::Module* temp_module = new Module::ConditionalPairDrawStack(variable_pair, condition_order, (";" + std::string(argv[6 + 2 * variable_num]) + ";arbitrary unit").c_str(), 50, std::stod(argv[7 + 2 * variable_num]), std::stod(argv[8 + 2 * variable_num]), (argv[5 + 2 * variable_num] + std::string("/") + argv[6 + 2 * variable_num] + ".png").c_str(), true, true, *loader.SignalLabel_address(), *loader.BackgroundLabel_address(), *loader.DataLabel_address(), *loader.MCLabel_address(), loader.Getvariable_names_address(), loader.VariableTypes_address());
        loader.InsertCustomizedModule(temp_module);
    }
