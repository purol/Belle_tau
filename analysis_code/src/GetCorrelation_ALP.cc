...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    double mass = std::stod(argv[7]);
    double life = std::stod(argv[8]);
    int A = std::stoi(argv[9]);
    int B = std::stoi(argv[10]);
    ...
    RooRealVar BDT2("BDT2", "BDT2", 0.0, 1.0);
    ...
    for (int i = 0; i < sample_list.size(); i++) loader_one.Load((argv[1] + std::string("/") + sample_list.at(i) + std::string("/") + std::string(argv[2])).c_str(), argv[3], "label");
    ...
    loader_one.Cut(cut_region.c_str());
    loader_one.PrintInformation("========== (-20 delta < M < 20 delta) && (-16 delta < deltaE < 6 delta) ==========");

    loader_one.Cut(cut_m_alpha.c_str());
    loader_one.PrintInformation("========== M_ALP cut ==========");

    loader_one.RandomBCS();
    loader_one.IsBCSValid();
    loader_one.PrintInformation("========== Random BCS ==========");
    ...
    loader_one.DrawTH2D("BDT1", "M", ";BDT1 output;M [GeV/c^{2}]", 50, 0.0, 1.0, 50, M_peak - 20 * M_left_sigma, M_peak + 20 * M_right_sigma, (argv[6] + std::string("/") + "BDT1_M_distribution_" + std::string(argv[7]) + "_" + std::string(argv[8]) + "_" + std::string(argv[9]) + "_" + std::string(argv[10]) + ".png").c_str(), "COLZ");
   
    RooDataSet dataset_one("dataset_one", "dataset_one", RooArgSet(BDT1, M_inv, weight), RooFit::WeightVar("weight"));
    loader_one.FillDataSet(&dataset_one, { &BDT1, &M_inv }, { "BDT1", "M" });
    ...
    for (int i = 0; i < sample_list.size(); i++) loader_two.Load((argv[1] + std::string("/") + sample_list.at(i) + std::string("/") + std::string(argv[2])).c_str(), argv[3], "label");
    ...
    loader_two.Cut(cut_region.c_str());
    loader_two.PrintInformation("========== (-20 delta < M < 20 delta) && (-16 delta < deltaE < 6 delta) ==========");

    loader_two.Cut(cut_m_alpha.c_str());
    loader_two.PrintInformation("========== M_ALP cut ==========");

    loader_two.RandomBCS();
    loader_two.IsBCSValid();
    loader_two.PrintInformation("========== Random BCS ==========");
    ...
    loader_two.DrawTH2D("BDT2", "M", ";BDT2 output;M [GeV/c^{2}]", 50, 0.0, 1.0, 50, M_peak - 20 * M_left_sigma, M_peak + 20 * M_right_sigma, (argv[6] + std::string("/") + "BDT2_M_distribution_" + std::string(argv[7]) + "_" + std::string(argv[8]) + "_" + std::string(argv[9]) + "_" + std::string(argv[10]) + ".png").c_str(), "COLZ");

    RooDataSet dataset_two("dataset_two", "dataset_two", RooArgSet(BDT2, M_inv, weight), RooFit::WeightVar("weight"));
    loader_two.FillDataSet(&dataset_two, { &BDT2, &M_inv }, { "BDT2", "M" });
    ...
    std::vector<TH1D*> M_BDT1_binning;
    std::vector<TH1D*> M_BDT2_binning;
    ...
    for (const auto& [lower, upper] : BDT_bins) {
        ...
        M_BDT1_binning.push_back(temp_th1d);
        ...
        loader_temp.PrintInformation("========== " + std::to_string(lower) + " < BDT1 < " + std::to_string(upper) + " ==========");
    ...
    for (const auto& [lower, upper] : BDT_bins) {
        TH1D* temp_th1d = new TH1D(("temp_th1d_" + std::to_string(lower) + "_" + std::to_string(upper)).c_str(), ";M [GeV/c^{2}]; arbitrary unit", 50, M_peak - 20 * M_left_sigma, M_peak + 20 * M_right_sigma);
        M_BDT2_binning.push_back(temp_th1d);
    ...
    for (auto& temp_hist : M_BDT1_binning) {
        temp_hist->Scale(1.0 / temp_hist->Integral());
    }
    for (auto& temp_hist : M_BDT2_binning) {
        temp_hist->Scale(1.0 / temp_hist->Integral());
    }
    ...
    TCanvas* canvas_M_BDT1_diff = new TCanvas("canvas_M_BDT1_diff", "canvas_M_BDT1_diff", 800, 800);
    TLegend* leg_M_BDT1_diff = new TLegend(0.65, 0.70, 0.88, 0.88);

    double maxY = 0.0;
    for (int i = 0; i < BDT_bins.size(); i++) {
        TH1D* temp_hist = M_BDT1_binning.at(i);
    ...
    for (int i = 0; i < BDT_bins.size(); i++) {
        ...
        leg_M_BDT1_diff->AddEntry(temp_hist, ("[" + std::to_string(lower) + ", " + std::to_string(upper) + "] GeV/c^{2}").c_str(), "l");
    }
    leg_M_BDT1_diff->SetBorderSize(0);
    leg_M_BDT1_diff->SetFillStyle(0);
    leg_M_BDT1_diff->Draw();

    canvas_M_BDT1_diff->Update();
    canvas_M_BDT1_diff->SaveAs((argv[6] + std::string("/") + "BDT1_M_diff_distribution_" + std::string(argv[7]) + "_" + std::string(argv[8]) + "_" + std::string(argv[9]) + "_" + std::string(argv[10]) + ".png").c_str());

    TCanvas* canvas_M_BDT2_diff = new TCanvas("canvas_M_BDT2_diff", "canvas_M_BDT2_diff", 800, 800);
    TLegend* leg_M_BDT2_diff = new TLegend(0.65, 0.70, 0.88, 0.88);

    double maxY = 0.0;
    for (int i = 0; i < BDT_bins.size(); i++) {
        TH1D* temp_hist = M_BDT2_binning.at(i);
        if (temp_hist->GetMaximum() > maxY) maxY = temp_hist->GetMaximum();
    }
    for (int i = 0; i < BDT_bins.size(); i++) {
        double lower = BDT_bins.at(i).first;
        ...
        leg_M_BDT2_diff->AddEntry(temp_hist, ("[" + std::to_string(lower) + ", " + std::to_string(upper) + "] GeV/c^{2}").c_str(), "l");
    }
    leg_M_BDT2_diff->SetBorderSize(0);
    leg_M_BDT2_diff->SetFillStyle(0);
    leg_M_BDT2_diff->Draw();

    canvas_M_BDT2_diff->Update();
    canvas_M_BDT2_diff->SaveAs((argv[6] + std::string("/") + "BDT2_M_diff_distribution_" + std::string(argv[7]) + "_" + std::string(argv[8]) + "_" + std::string(argv[9]) + "_" + std::string(argv[10]) + ".png").c_str());

    // print result
    printf("weighted pearson correlation between BDT1 and M on (-20 delta < M < 20 delta) && (-5 delta < deltaE < 5 delta): %lf\n", dataset_one.correlation(BDT1, M_inv));
    printf("weighted pearson correlation between BDT2 and M on (-20 delta < M < 20 delta) && (-15 delta < deltaE < -5 delta): %lf\n", dataset_two.correlation(BDT2, M_inv));
