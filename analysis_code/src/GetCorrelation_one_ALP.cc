...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    loader.DrawTH2D("BDT_output_1", "M", ";BDT1 output;M [GeV/c^{2}]", 50, 0.0, 1.0, 50, M_peak - 20 * M_left_sigma, M_peak + 20 * M_right_sigma, (argv[6] + std::string("/") + "BDT1_M_distribution_" + std::string(argv[7]) + "_" + std::string(argv[8]) + "_" + std::string(argv[9]) + "_" + std::string(argv[10]) + ".png").c_str(), "COLZ");
   
    RooDataSet dataset_one("dataset_one", "dataset_one", RooArgSet(BDT1, M_inv, weight), RooFit::WeightVar("weight"));
    loader.FillDataSet(&dataset_one, { &BDT1, &M_inv }, { "BDT_output_1", "M" });
