...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    ReadResolution((std::string(argv[5]) + "/alpha_mass" + std::format("{:g}", mass) + "_life" + std::format("{:g}", life) + "_A" + std::to_string(A) + "_B" + std::to_string(B) + "_M_deltaE_result.txt").c_str(), &deltaE_peak, &deltaE_left_sigma, &deltaE_right_sigma, &M_peak, &M_left_sigma, &M_right_sigma, &theta);
    ...
    loader.DrawTH2D("BDT1", "M", ";BDT1 output;M [GeV/c^{2}]", 50, 0.0, 1.0, 50, M_peak - 20 * M_left_sigma, M_peak + 20 * M_right_sigma, (argv[6] + std::string("/") + "BDT1_M_distribution_" + std::string(argv[7]) + "_" + std::string(argv[8]) + "_" + std::string(argv[9]) + "_" + std::string(argv[10]) + ".png").c_str(), "COLZ");
   
    RooDataSet dataset_one("dataset_one", "dataset_one", RooArgSet(BDT1, M_inv, weight), RooFit::WeightVar("weight"));
    loader.FillDataSet(&dataset_one, { &BDT1, &M_inv }, { "BDT1", "M" });
    ...
    canvas_M_BDT_diff->SaveAs((argv[6] + std::string("/") + "BDT1_M_diff_distribution_" + std::string(argv[7]) + "_" + std::string(argv[8]) + "_" + std::string(argv[9]) + "_" + std::string(argv[10]) + ".png").c_str());
