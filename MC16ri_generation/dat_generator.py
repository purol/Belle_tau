...
# Parameter sets
masses = [round(0.3 + 0.2 * i, 1) for i in range(0, 7)]  # 0.3 to 1.5 GeV
lifetimes = [0.1, 1, 10, 100, 250, 500, 1000]  # mm
coefficients = [(1, -1)]  # (A_COEF, B_COEF)

# Directories
dat_dir = "dat_files"
os.makedirs(dat_dir + "_plus", exist_ok=True)
os.makedirs(dat_dir + "_minus", exist_ok=True)

# Template
dat_template_plus = """0.08711                         * c*tau(tau life time) (mm) 
...
dat_template_minus = """0.08711                         * c*tau(tau life time) (mm) 
1                               * switch for long lived (1:no decay, 0:decay)
1                               * switch for KKMC-JETSET (1:on, 0:off)
********** Above is for basf *********
...
********************************************************************************
  901              4      Ihvp  ! =1,2,3,4
********************************************************************************
 2001          373e0      Jak1   =xpar(71)  First  Tau decay mask (tau-)
 2002            0e0      Jak2   =xpar(72)  Second Tau decay mask (tau+)
********************************************************************************
 2900            1e0      BBB    0: ORIG  1: BBB
 2901            0e0      EQUALBR
 2902            2e0      FF2PIRHO
...
for mass in masses:
    for lifetime in lifetimes:
        for a_coef, b_coef in coefficients:
            filename = f"{dat_dir}_plus/alpha_mass{mass:.1f}_life{lifetime}_A{a_coef}_B{b_coef}_plus.dat"
            with open(filename, "w") as f:
                f.write(dat_template_plus.format(mass=mass, lifetime=lifetime, a_coef=a_coef, b_coef=b_coef))
            dat_files.append(filename)

print(f"Generated {len(dat_files)} .dat files in {dat_dir}_plus/")
...
for mass in masses:
    for lifetime in lifetimes:
        for a_coef, b_coef in coefficients:
            filename = f"{dat_dir}_minus/alpha_mass{mass:.1f}_life{lifetime}_A{a_coef}_B{b_coef}_minus.dat"
            with open(filename, "w") as f:
                f.write(dat_template_minus.format(mass=mass, lifetime=lifetime, a_coef=a_coef, b_coef=b_coef))
            dat_files.append(filename)

print(f"Generated {len(dat_files)} .dat files in {dat_dir}_minus/")
