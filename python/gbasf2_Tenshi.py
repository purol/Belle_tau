...
def DefineVariables(tau_list, photon_names, IsItNominal, path):
    # define vars
    sig_vars = ["Mbc", "deltaE"] + \
    ...
    if(IsItNominal):
        ...
        variable_list = ["dr", "dz"] + \
                        ["muonID", "electronID", "pionID", "pionIDNN"] + \
                        ["charge"] + \
...
tauLFVCuts_control = "nParticlesInList(pi+:taulfv) < 7 and 0.5 < M < 2.0 and -1.0 < deltaE < 0.5"
...
if(args.prompt):
    ...
    # define variables
    var_list = DefineVariables("tau+:LFV_lll", photon_names=photon_names, IsItNominal=True, path=my_path)
...
if(args.vertex):
    ...
    var_list = DefineVariables("tau+:LFV_vertex", photon_names=photon_names, IsItNominal=False, path=my_path)
