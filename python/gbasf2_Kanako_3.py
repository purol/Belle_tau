...
if tau_list:
    ...
    if(args.gencut): # make Ntuple for gencut
        ...
        Condition_one = '[[daughter(0, p) > daughter(1, p)] and [daughter(0, p) > daughter(2, p)] and [daughter(0, muonID) > 0.5]]'
        Condition_two = '[[daughter(1, p) > daughter(0, p)] and [daughter(1, p) > daughter(2, p)] and [daughter(1, muonID) > 0.5]]'
        Condition_three = '[[daughter(2, p) > daughter(0, p)] and [daughter(2, p) > daughter(1, p)] and [daughter(2, muonID) > 0.1]]'
        ma.applyCuts('tau+:tau_3mu_mumumu', Condition_one + ' or ' + Condition_two + ' or ' + Condition_three, path=my_path)
        MDeltaCuts = ["", "[M < 3.5] and [-1.1 < deltaE < 1.0]", "[1.3 < M < 2.2] and [-1.0 < deltaE < 0.5]"]
        AnalysisGenCut(tau_list = "tau+:tau_3mu_mumumu", MDeltaCuts = MDeltaCuts, Ntuple_name = output_file, path=my_path)
