...
def AssignIndex(sample_name, event_name, energy_name):
    ...
    if(event_name=="data"):
        ...
        type_index = 33
    elif(event_name=="hhISR"):
        type_index = 34
    elif(event_name=="llXX"):
        type_index = 35
    elif(event_name=="udsc"):
        type_index = 36
    else:
...
def DefineVariables(tau_list, photon_names, IsItPrompt, path):
    ...
    if(IsItPrompt):
        ...
        ma.variableToSignalSideExtraInfo(tau_list + "_ROE", {"nROE_RemainingTracks(cleanMask)":"nROE_RemainingTracks_cleanMask"}, path=roe_path_prompt)
        ...
        ma.variableToSignalSideExtraInfo(tau_list + "_ROE", {"nROE_RemainingTracks(cleanMask)":"nROE_RemainingTracks_cleanMask"}, path=roe_path_ALP)
    ...
    if BELLEONE:
        ...
        pass
    tag_vars = vc.recoil_kinematics + GetROEVariables("cleanMask") + ["extraInfo(nROE_RemainingTracks_cleanMask)"]
...
def AnalysisGenCut(tau_list, MDeltaCuts, Ntuple_name, path):
    # names
    output_file = Ntuple_name + ".root"
    ...
    ma.rankByLowest(particleList="tau+:fake", variable="abs(deltaE)", outputVariable='bcs_deltaE', path=path)
    ma.rankByLowest(particleList="tau+:fake", variable="abs(dM)", outputVariable='bcs_dM', path=path)
    ma.cutAndCopyList(outputListName = "tau+:BCS_deltaE", inputListName = "tau+:fake", cut = "extraInfo(bcs_deltaE) == 1", path=path)
    ma.cutAndCopyList(outputListName = "tau+:BCS_dM", inputListName = "tau+:fake", cut = "extraInfo(bcs_dM) == 1", path=path)

    ma.cutAndCopyList(outputListName = "tau+:fake_strict", inputListName = "tau+:fake", cut = MDeltaCuts, path=path)
    ma.rankByLowest(particleList="tau+:fake_strict", variable="abs(deltaE)", outputVariable='bcs_deltaE', overwriteRank = True, path=path)
    ma.rankByLowest(particleList="tau+:fake_strict", variable="abs(dM)", outputVariable='bcs_dM', overwriteRank = True, path=path)
    ma.cutAndCopyList(outputListName = "tau+:BCS_strict_deltaE", inputListName = "tau+:fake_strict", cut = "extraInfo(bcs_deltaE) == 1", path=path)
    ma.cutAndCopyList(outputListName = "tau+:BCS_strict_dM", inputListName = "tau+:fake_strict", cut = "extraInfo(bcs_dM) == 1", path=path)
    ...
    variable_list = variable_list + ["sumValueInList(tau+:BCS_strict_dM, dM)", "sumValueInList(tau+:BCS_strict_dM, M)", "sumValueInList(tau+:BCS_strict_dM, deltaE)", "sumValueInList(tau+:BCS_strict_dM, p)", "sumValueInList(tau+:BCS_strict_dM, Mbc)"]
    ...
    variable_list = variable_list + ["sumValueInList(tau+:BCS_strict_deltaE,R2)", "sumValueInList(tau+:BCS_strict_deltaE,cosTBTO)", "sumValueInList(tau+:BCS_strict_deltaE,cosTBz)", "sumValueInList(tau+:BCS_strict_deltaE,thrustBm)", "sumValueInList(tau+:BCS_strict_deltaE,thrustOm)"]
    variable_list = variable_list + ["sumValueInList(tau+:BCS_strict_deltaE,daughterHighest(p))", "sumValueInList(tau+:BCS_strict_deltaE,daughterLowest(p))", "sumValueInList(tau+:BCS_strict_deltaE,daughterHighest(E))", "sumValueInList(tau+:BCS_strict_deltaE,daughterLowest(E))"]
    variable_list = variable_list + ["sumValueInList(tau+:BCS_strict_dM,R2)", "sumValueInList(tau+:BCS_strict_dM,cosTBTO)", "sumValueInList(tau+:BCS_strict_dM,cosTBz)", "sumValueInList(tau+:BCS_strict_dM,thrustBm)", "sumValueInList(tau+:BCS_strict_dM,thrustOm)"]
    variable_list = variable_list + ["sumValueInList(tau+:BCS_strict_dM,daughterHighest(p))", "sumValueInList(tau+:BCS_strict_dM,daughterLowest(p))", "sumValueInList(tau+:BCS_strict_dM,daughterHighest(E))", "sumValueInList(tau+:BCS_strict_dM,daughterLowest(E))"]
...
parser.add_argument('--gencut', action='store_true', help='This flag allows you to gen cut study')
...
if tau_list:
    ...
    if(args.gencut): # make Ntuple for gencut
        tauCutsForGenStudy = "[1.60 < M < 1.94] and [-0.45 < deltaE < 0.39] and [roeEextra(cleanMask) < 5.0] and [extraInfo(OneMuon_muonID) > 0.1] and [extraInfo(TwoMuon_muonID) > 0.1]"
        ma.applyCuts("tau+:LFV_comb", cut = tauCutsForGenStudy, path=my_path)
        AnalysisGenCut(tau_list = "tau+:LFV_comb", MDeltaCuts = "[1.0 < M < 4.0] and [-1.0 < deltaE < 1.0]", Ntuple_name = output_file, path=my_path)
