...
if tau_list:
    ...
    if(args.gencut): # make Ntuple for gencut
        tauCutsForGenStudy = "[1.60 < M < 1.94] and [-0.45 < deltaE < 0.39] and [extraInfo(OneMuon_muonID) > 0.1] and [extraInfo(TwoMuon_muonID) > 0.1] and [extraInfo(ThreeMuon_muonID) > 0.1]"
        ma.applyCuts("tau+:LFV_comb", cut = tauCutsForGenStudy, path=my_path)
        MDeltaCuts = ["", "[M < 3.5] and [-1.1 < deltaE < 1.0]", "[1.55 < M < 1.99] and [-0.50 < deltaE < 0.44]"]
        AnalysisGenCut(tau_lists = ["tau+:LFV_comb"], MDeltaCuts = MDeltaCuts, Ntuple_name = output_file, path=my_path)
