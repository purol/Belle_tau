...
if(args.vertex):
    pdg.add_particle('alpha', 94144, 999., 0.004, 0, 0)  # name, PDG, mass, width, charge, spin
    ma.reconstructDecay("alpha:LFV_vertex -> mu+:all mu-:all", cut="", path=my_path)
    ma.reconstructDecay("tau+:LFV_vertex -> alpha:LFV_vertex mu+:taulfv", tauLFVCuts3, 10, path=my_path)
