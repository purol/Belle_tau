...
if tau_list:
    ...
    if(args.gencut): # make Ntuple for gencut
        ...
        ma.cutAndCopyList("tau+:LFV_comb_2", "tau+:LFV_comb", cut="[1.60 < M < 1.94] and [-0.45 < deltaE < 0.39] and [roeEextra(cleanMask) < 5.0] and [extraInfo(OneMuon_muonID) > 0.1]", path=my_path)
        ma.cutAndCopyList("tau+:LFV_comb_3", "tau+:LFV_comb", cut="[1.60 < M < 1.94] and [-0.45 < deltaE < 0.39] and [roeEextra(cleanMask) < 5.0] and [extraInfo(OneMuon_muonID) > 0.1] and [extraInfo(TwoMuon_muonID) > 0.1]", path=my_path)
