...
def AnalysisGenCut(tau_list, Ntuple_name, path):
    ...
    ma.rankByLowest(particleList="tau+:fake", variable="abs(deltaE)", outputVariable='bcs', path=path)
    ma.cutAndCopyList(outputListName = "tau+:BCS", inputListName = "tau+:fake", cut = "extraInfo(bcs) == 1", path=path)
...
if tau_list:
    ...
    if(args.gencut): # make Ntuple for gencut
        ...
        AnalysisGenCut(tau_list = "tau+:LFV_comb", Ntuple_name = output_file, path=my_path)
