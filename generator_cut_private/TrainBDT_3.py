...
#
# Region A:
# tau+:fake_strict0 candidate가 존재
# ------------------------------------------------------------
...
#
# Region B:
# tau+:fake candidate는 존재하지만
# tau+:fake_strict0 candidate는 존재하지 않음
# ------------------------------------------------------------
...
# ============================================================
# A/B/C threshold 동시 최적화
# ============================================================

def optimize_three_thresholds(
    results: dict[str, dict],
    efficiency_target: float,
    n_scan: int,
) -> dict:
    """
    Region A/B/C의 threshold를 동시에 최적화한다.

    A와 B threshold를 loop로 scan하고,
    각 A/B 조합에서 목표 efficiency를 만족하는
    가장 높은 C threshold를 선택한다.
    """
    ...
    data_a = threshold_data["A"]
    data_b = threshold_data["B"]
    data_c = threshold_data["C"]
    ...
    for index_a, threshold_a in enumerate(
        ...
        signal_a = (
            data_a["signal_pass"][index_a]
        )

        background_a = (
            data_a["background_pass"][index_a]
        )

        for index_b, threshold_b in enumerate(
            data_b["thresholds"]
        ):
            signal_ab = (
                signal_a
                + data_b["signal_pass"][index_b]
            )
            ...
            valid_c_indices = np.flatnonzero(
                signal_efficiencies
                >= efficiency_target
            )
            ...
            # threshold 배열은 오름차순이다.
            # 목표 효율을 만족하는 가장 높은 C threshold를 선택
            index_c = valid_c_indices[-1]
    ...
    if best is None:
        raise RuntimeError(
            "No A/B/C threshold combination "
...
def calculate_combined_roc(
    ...
    """
    A/B/C threshold를 독립적으로 scan하여 combined ROC의
    envelope를 계산한다.
    """
    ...
    if (
        n_signal_total == 0
        or n_background_total == 0
    ):
        raise RuntimeError(
            "Cannot calculate combined ROC "
            "with empty signal or background."
        )

    best_rejection_by_signal_count = {}

    for index_a in range(
        len(data_a["thresholds"])
    ):
        signal_a = (
            data_a["signal_pass"][index_a]
        )
...
def main() -> None:
    ...
    best = optimize_three_thresholds(
        results=results,
        efficiency_target=(
            efficiency_target
        ),
