...
submit_GridSearch() {
  ...
  get_params "./${VerName}/${Analysis_VerName}/ALP/final_output" | while read mass life A B; do
    ...
    bsub -q s \
    -J FBDTTRN \
    -o "./${VerName}/${Analysis_VerName}/${OutputPath}/log_${mass}_${life}_${A}_${B}/${nTree}_${depth}_${shrinkage}_${subsample}_${binning}.log" \
    -e "./${VerName}/${Analysis_VerName}/${OutputPath}/err_${mass}_${life}_${A}_${B}/${nTree}_${depth}_${shrinkage}_${subsample}_${binning}.err" \
    ${Code} \
    "${#input_variables_ref[@]}" \
    "${input_variables_ref[@]}" \
