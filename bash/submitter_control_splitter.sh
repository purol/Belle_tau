...
export Analysis_Name="Larva_CTRL"
...
submit_splitter() {
  ...
  for file in "${Ntuple_DIR}/${VerName}/${SampleName}/MC16rd"/*.root; do
    filename=$(basename "$file" .root) # without path, without extension
    bsub -q s \
    -J Split \
    -o "./${VerName}/${Analysis_VerName}/${SampleName}/log/${filename}.log" \
    -e "./${VerName}/${Analysis_VerName}/${SampleName}/err/${filename}.err" \
    ${Code} \
    "${Ntuple_DIR}/${VerName}/${SampleName}/MC16rd" \
    "${filename}.root" \
    "./${VerName}/${Analysis_VerName}/${SampleName}/output/" \
    "./${VerName}/${Analysis_VerName}/${SampleName}/output/"
    sleep 0.3s
  done
