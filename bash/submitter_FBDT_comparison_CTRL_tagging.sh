...
submit_Plotter() {
  ...
  "${VarMin}" \
  "${VarMax}" \
  "./${VerName}/${Analysis_VerName}/${Signal_Type}/${InputDir}/" \
  "./${VerName}/${Analysis_VerName}/${OutputPath}/" \
  "${OutputName}.png" \
  "${Type}"
}
 
code="${Belle_tau_DIR}/analysis_code/bin/tag_comparison_CTRL"
VarName="BDT_output_1"
submit_Plotter ${code} ${Analysis_Name} ${VarName} 0.0 1.0 "final_output_after_application" "FBDT1_comp" "plot" "${Signal_Type}"

code="${Belle_tau_DIR}/analysis_code/bin/tag_comparison_CTRL"
VarName="BDT_output_2"
submit_Plotter ${code} ${Analysis_Name} ${VarName} 0.0 1.0 "final_output_after_application" "FBDT2_comp" "plot" "${Signal_Type}"