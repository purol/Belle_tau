...
submit_GridSearch() {
  ...
  get_params "./${VerName}/${Analysis_VerName}/ALP/final_output" | while read mass life A B; do
    ...
    bsub -q l \
    -J AUCTRN \
    -o "./${VerName}/${Analysis_VerName}/${OutputPath}/log_AUC_train_${mass}_${life}_${A}_${B}/${nTree}_${depth}_${shrinkage}_${subsample}_${binning}.log" \
    -e "./${VerName}/${Analysis_VerName}/${OutputPath}/err_AUC_train_${mass}_${life}_${A}_${B}/${nTree}_${depth}_${shrinkage}_${subsample}_${binning}.err" \
    ${Code} \
    "${#input_variables_ref[@]}" \
    "${input_variables_ref[@]}" \
    "./${VerName}/${Analysis_VerName}" \
    "./${VerName}/${Analysis_VerName}/${OutputPath}/out_${mass}_${life}_${A}_${B}" \
    "${nTree}" \
    "${depth}" \
    "${shrinkage}" \
    "${subsample}" \
...
code="${Belle_tau_DIR}/analysis_code/bin/FBDT_AUC_train_one_ALP"
output="GridSearch_one"
for nTree in 250 500 750 2000
do
  for depth in 1 2
  do
...
code="${Belle_tau_DIR}/analysis_code/bin/FBDT_AUC_train_two_ALP"
output="GridSearch_two"
for nTree in 100 250 500 750 2000
do
  for depth in 1 2
  do
    for shrinkage in 0.01 0.1 0.2
