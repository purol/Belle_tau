...
submit_GridSearch() {
  ...
  bsub -q l -J AUCTRN -o "./${VerName}/${Analysis_VerName}/${OutputPath}/log_AUC_train/${nTree}_${depth}_${shrinkage}_${subsample}_${binning}.log" -e "./${VerName}/${Analysis_VerName}/${OutputPath}/err_AUC_train/${nTree}_${depth}_${shrinkage}_${subsample}_${binning}.err" ${Code} "${#input_variables_ref[@]}" "${input_variables_ref[@]}" "./${VerName}/${Analysis_VerName}" "./${VerName}/${Analysis_VerName}/${OutputPath}/out" "${nTree}" "${depth}" "${shrinkage}" "${subsample}" "${binning}" "${Signal_Type}" "${Background_Types_STR}"
